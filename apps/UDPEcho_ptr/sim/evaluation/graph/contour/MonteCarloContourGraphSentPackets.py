...
class MonteCarloContourGraphSentPackets:
    def __init__(self):
        pass
    ...
    def execute(self,
        ...
        print "="*40
        print "Executing MonteCarloContourGraphSentPackets:"
        print "filenamebase\t\t", si.create_montecarlo_filenamebase()
        print "="*40

        total_sent_packets = np.zeros(si.nodes+2)
        ...
        for run in range(0, iterations):
            ...
            sent_packets_run = np.load(monte_si.createfilenamebase() + \
                                           "_sent_packets_per_node.npy",
                                  "r")
            #print "cr", consist_run
            total_sent_packets += sent_packets_run

        total_sent_packets = total_sent_packets / iterations
        ...
        print ">>>>x", xarr
        print ">>>>y", yarr
        print ">>>>sent_packets", total_sent_packets

        xi = np.linspace(0, max(xarr), 100)
        yi = np.linspace(0, max(yarr), 100)
        zi = griddata(xarr[2:], yarr[2:], total_sent_packets[2:], xi, yi)
        ...
        plt.savefig(si.create_montecarlo_filenamebase()+"_montecarlo_contour_sent_packets.pdf")
